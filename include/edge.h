...
class edge {
    ...
    Station * refStn;
    int lastTreeIndex;
    int branchRootIndex; // the index of the first branch root before this edge
    bool operator==(const edge& c) throw ();
    edge(int,int, Station*);
    edge(int,int, Station*, int);
    edge();

    bool operator < (const edge& e) const;
    static bool compare_lastoccurrence ( const edge& a, const edge& b );
};
