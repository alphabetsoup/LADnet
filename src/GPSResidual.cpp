...
GPSResidual::GPSResidual() {
}
GPSResidual::GPSResidual(::GPSBaseline * bl, vector<double> tuple) {
  _bl = bl;
...
void GPSResidual::standardise() {
    ...
    */
    //for (int i=0;i<3;i++) _tuple_st_ENH[i] = sqrt(abs(rot_raw_sq(i)*_rVCVi(i,i))); //orig
    for (int i=0;i<3;i++) _tuple_st_ENH[i] = sqrt(abs(rot_raw_sq(i)/_rVCV(i,i)));
}


int GPSResidual::valuesToString(vector< string >& fv) {
...
int GPSResidual::typesToString(vector< string >& t) {
    t.push_back("double");
    t.push_back("double");
    t.push_back("double");
    t.push_back("double");
