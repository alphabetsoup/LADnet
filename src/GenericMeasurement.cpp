...
#include "global.h"
#include "GenericMeasurement.h"
#include "Station.h"
#include "ParameterGroup.h"
#include "GenericComputedObservable.h"
...
GenericMeasurement::GenericMeasurement(int id) : DnaMeasurement(id) {
}

GenericMeasurement::GenericMeasurement(int id, double value) : DnaMeasurement(id) {
...
GenericMeasurement::~GenericMeasurement()
{
	...
	for (std::map<int,GenericComputedObservable*>::iterator cit = X.begin(); cit != X.end(); cit++) delete cit->second;
	for (std::map<int,GenericResidual*>::iterator           rit = V.begin(); rit != V.end(); rit++) delete rit->second;
}



void GenericMeasurement::setRawVCV(double stddev)
{
	_StdDev = stddev;
}
double GenericMeasurement::getRawVCV()
{
	return _StdDev;
}

void GenericMeasurement::printRawVCV(ostream *_logstream) {
...
void GenericMeasurement::printSVCV(ostream *_logstream) {
        *_logstream << "sVCV for meas " << measID << endl << sVCV <<endl;
}

int GenericMeasurement::valuesToString(vector< string >& fv) {
...
int GenericMeasurement::namesToString(vector< string >& fn) {
...
int GenericMeasurement::typesToString(vector< string >& t) {
    t.push_back("int");
    t.push_back("string");
    t.push_back("string");
    t.push_back("string");
 ...
 */
int GenericMeasurement::calculateForSegment(int segID) {
	...
	GenericComputedObservable * c = new GenericComputedObservable(this, calc); // DYNAMIC
	X[segID] = c;

    // inevitibly, this is where the residual is computed.
	vector< double > residual;
	for (int i=0;i<calc.size(); i++) residual.push_back(_components[i]-calc[i]);
	GenericResidual * v = new GenericResidual(this,residual); // DYNAMIC
	V[segID] = v;

	return 0;
}

int GenericMeasurement::getColCount(int segID) { 
	return 0; // fixme
...
int GenericMeasurement::getRowCount(int segID) { 
	return 0; // FIXME
}

// indices relative to measurement, not Jacobian
double GenericMeasurement::getPartial(int row, int column, int segID) {
	if (!_param.size()) throw domain_error(static_cast<ostringstream*>( &(ostringstream()
	                          << "GenericMeasurement::getParial() No parameters have been set for this GenericMeasurement."
	                         ) )->str());
	...
	while (column >= 0) {
		if (pit == _param.end()) throw domain_error(static_cast<ostringstream*>( &(ostringstream()
	                          << "GenericMeasurement::getPartial() Column index out of bounds for this GenericMeasurement."
	                         ) )->str());
    ...
    if ((*pit)->hasIndicesForSegment(segID)) {
		...
		throw domain_error(static_cast<ostringstream*>( &(ostringstream()
	                          << "GenericMeasurement::getPartial() No parameter indices exist for parameter ID " << (*pit)->_id << " in this GenericMeasurement."
	                         ) )->str());
	}
}

double GenericMeasurement::getObserved(int row, int segID) {
...
int GenericMeasurement::getBasePoint(double Xc[3]) {
...
void GenericMeasurement::toKMLElement(ostream* kmlfile, int segID, bool principle, KMLSpec * spec) {
	...
	std::cout << "Getting residuals...";
     ...
     GenericResidual * res;
     try {
         res = (GenericResidual*)(V.at(segID));
...
void GenericMeasurement::toCSVRow(ostream * csvfile, int segID) {
     vector< string > mfv;
...
void GenericMeasurement::getCSVHeader(ostream * csvfile) {
...
void GenericMeasurement::getKMLHeader(ostream * kmlfile) {
    vector< string > mfn, mft, rfn, rft;
	namesToString(mfn);
	typesToString(mft);
	// same for residual
	// if no residuals in this segment, we're out of luck.
	GenericResidual dummyr;
...
void GenericMeasurement::toShp(DBFHandle hDBF, SHPHandle hSHP, int segID) {
    SHPObject   *psObject;
    ...
    // compare number of fields in dbf to fields in GenericMeasurement
    int totalfields = values.size();
    if( DBFGetFieldCount( hDBF ) !=  totalfields ) {
        string errorstr = "Mismatch: number of fields in DBF does not equal number of fields in GenericMeasurement.";
...
SHPHandle GenericMeasurement::createSHP(string shpname) {
    int nShapeType = SHPT_ARC;
    SHPHandle hSHP = SHPCreate( shpname.c_str(), nShapeType );
    return hSHP;
}

DBFHandle GenericMeasurement::createDBF(string dbfname) {
    vector< string > fn;
    vector< string > ft;

    DBFHandle hDBF = DBFCreate( dbfname.c_str() );
    namesToString(fn);
    GenericResidual::namesToString(fn); 
    GenericComputedObservable::namesToString(fn); 
    typesToString(ft);
    GenericResidual::typesToString(ft);
    GenericComputedObservable::typesToString(ft); 
...
std::string GenericMeasurement::getLabel() {
	return _param[0]->name + " to " + _param[1]->name;
}

//void GenericMeasurement::scaleVCV(EllipsoidModel * ellip) {
void GenericMeasurement::scaleVCV(double a, double e) {
...
void GenericMeasurement::CholeskyDecomposeSVCV()
{
...
void GenericMeasurement::printCholDecomSVCV(std::ostream *_logstream)
{
}

void GenericMeasurement::checkCholSVCV(ostream *_logstream) {
...
Residual * GenericMeasurement::getResidualForSegment(int segID) {
	assert(V.find(segID) != V.end());
	return (Residual *)V[segID];
}

void GenericMeasurement::useDefaultVCV(double N, double E, double H, double ppm, double a, double e) {
	// set scales to 1;
